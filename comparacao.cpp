...
#pragma omp critical
    ...
    std::cout << "----------------Bhaskara com Atomic, Critical e Barrier---------------\n";
    double soma_totalBarrier = 0.0;

    // Vetor para armazenar o tempo gasto por thread
    double T0B = omp_get_wtime(); // tempo inicial
...
#pragma omp critical
            {
                soma_totalBarrier += local_x1[i] + local_x2[i];
                std::cout << "Equação " << i + 1 << ": "
                          << a[i] << "x^2 + " << b[i] << "x + " << c[i] << " = 0"
    ...
    double T1B = omp_get_wtime();
    double duracaoBarrier = T1B - T0B;

    std::cout << "\nSoma total de todas as raízes: " << soma_totalBarrier << std::endl;
    std::cout << "Tempo total: " << duracaoBarrier << " ms" << std::endl;
    std::cout << "----------------------------------------\n";

    std::cout << "-----------Bhaskara com Atomic, Critical, Barrier e Ordered-----------\n";

    double soma_totalOrdered = 0.0;

    // Vetor para armazenar o tempo gasto por thread
    double T0O = omp_get_wtime(); // tempo inicial
...
#pragma omp ordered
            {
// Protege soma_totalOrdered com critical
#pragma omp critical
                soma_totalOrdered += local_x1[i] + local_x2[i];
    ...
    double T1O = omp_get_wtime();
    double duracaoOrdered = T1O - T0O;

    std::cout << "\nSoma total de todas as raízes: " << soma_totalOrdered << std::endl;
    std::cout << "Tempo total: " << duracaoOrdered << " ms" << std::endl;
    std::cout << "----------------------------------------\n";

    std::cout << "\n======================= RELATÓRIO COMPARATIVO =======================\n";
    std::cout << "Número de equações resolvidas: " << N << "\n\n";
